...
def default_selected_metrics() -> List[str]:
  ...
  metrics += [f'test_{m}' for m in _SPLIT_METRICS]

  # Out of distribution metrics
  metrics += [
      f'{dset}_{m}'
      for (dset, m
          ) in itertools.product(['cifar_10h', 'imagenet_real'], _SPLIT_METRICS)
  ]
...
def row_wise_unique_non_nan(df: pd.DataFrame) -> pd.Series:
  """Checks there is only one non-NA value in each row, and returns its value."""
...
def _fill_upstream_test_metrics(df: pd.DataFrame) -> pd.DataFrame():
  ...
  Returns:
    A copy of `df` where the rows corresponding to models trained on upstream
      datasets have their in-distribution test metrics filled with
      in-distribution validation metrics.
  """
  df = df.copy()
